...
bool ArmorDetector::armorCompare(const ArmorBox & a_armor, const ArmorBox & b_armor, const ArmorBox & lastArmor, const int & targetNum)
{
	...
	// /// Method1: score by area & distancce
	...
	/// Method2: (lzj) score by pixel distance from center
	int c_x = 0.5 * this->srcImg.cols;
	int c_y = 0.5 * this->srcImg.rows;

	a_score = -( pow((a_armor.center.x - c_x),2) + pow((a_armor.center.y - c_y),2) );
	b_score = -( pow((b_armor.center.x - c_x),2) + pow((b_armor.center.y - c_y),2) );

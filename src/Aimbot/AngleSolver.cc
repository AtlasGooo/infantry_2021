...
void AngleSolver::solveAngles()
{
	...
	current_x_ = tVec.at<double>(0, 0);
	current_y_ = tVec.at<double>(1, 0);
	current_z_ = tVec.at<double>(2, 0);

	distance = sqrt(current_x_ * current_x_ + current_y_ * current_y_ + current_z_ * current_z_);


	/// (lzj) change into single method
	P4P_solver();	
...
void AngleSolver::compensateGravity()
{
	...
	float compensate_ratio = 0.12;
	float compensateGravity_pitch_tan = tan(x_pitch/180*CV_PI) + (compensate_ratio * 0.5*9.8*(distance / BULLET_SPEED) );
    x_pitch = atan(compensateGravity_pitch_tan)/CV_PI*180;
}

void AngleSolver::compensateHorizontal(){

	float comp_hori_ratio = -0.0001;
	y_yaw = y_yaw + comp_hori_ratio * distance;
}
...
void AngleSolver::getAngle(vector<Point2f>& contourPoints,Point2f cenerPoint, ArmorType type, double & yaw, double & pitch, double & evaluateDistance)
{
	...
	/// (lzj) add set pos for filter
	p_filter_->setCurrentPos(current_x_,current_y_,current_z_);

    compensateAngle();

	/// (lzj) add compensateHorizontal()
	compensateHorizontal();

	/// (lzj) set yaw pitch to filter
	p_filter_->setYawPitch(y_yaw,x_pitch);	


	/// (lzj)(TODO)
	compensateProbFilter();
	if(beliefCoef_  >  p_filter_->getfuseThreash()){
		beliefCoef_ = 1;
	}

	// beliefCoef_ = 1;

	yaw = y_yaw * beliefCoef_;
	pitch = x_pitch * beliefCoef_;
...
void AngleSolver::setFilter3D(ProbFilter3D * filter){
	this->p_filter_ = filter;
}

void AngleSolver::compensateProbFilter(){
	beliefCoef_ = this->p_filter_->calcProb();
	this->p_filter_->putArmorPos();

	/// (test)
	// #ifdef DEBUG_FILTER
	// 	cout << "belief: " << beliefCoef_ << endl;
	// #endif

...
void ArmorDetector::setImg(Mat & src){
	...
	/// Method2: HSV (lzj)
	cv::Mat srcImg_HSV,red_bin_1,red_bin_2;
	cv::cvtColor(srcImg,srcImg_HSV,COLOR_BGR2HSV);
	...
	if(enemyColor==RED){
		...
		cv::inRange(srcImg_HSV,cv::Scalar(0,rd_lw_s,rd_lw_v),cv::Scalar(15,255,255),red_bin_1);
		cv::inRange(srcImg_HSV,cv::Scalar(150,rd_lw_s,rd_lw_v),cv::Scalar(180,255,255),red_bin_2);
		cv::bitwise_or(red_bin_1,red_bin_2,srcImg_binary);
	}
	else if(enemyColor==BLUE){
		/// (lzj) blue: 100~124
		cv::inRange(srcImg_HSV,cv::Scalar(90,bl_lw_s,bl_lw_v),cv::Scalar(124,255,255),srcImg_binary);
	}

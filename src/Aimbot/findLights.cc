...
void ArmorDetector::findLights() {
	...
	for (const auto& lightContour : lightContours) {
		...
		/// (lzj) add rules for light
		// if(lightRect.size.width > lightRect.size.height*1.1 ) continue;
		
		cv::Point2f pts[4];
		lightRect.points(pts);
		if( abs(pts[2].x - pts[0].x) > abs(pts[2].y - pts[0].y) ){
			continue;
		}

		if( (lightRect.boundingRect().area() > 1000) && (abs(pts[2].x - pts[0].x) > abs(pts[2].y - pts[0].y)*0.7) )
			continue;

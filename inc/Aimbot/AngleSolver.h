...
/// (lzj) add probFilter3D to AngleSolver
...
public:
	...
	*/
	...
	/// (lzj) add a step to compensate horizontal difference for specify vehicle
	void compensateHorizontal();
    ...
    */
	...
	/// (lzj) add a function to set filter
	/**
	 * @brief set filter
	 * @param {ProbFilter3D *} filter A pointer to a filter to put in
	 * @return {*}
	 */
	void setFilter3D(ProbFilter3D * filter);

	void compensateProbFilter();
